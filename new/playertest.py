...
class VideoMain(QMainWindow):
    ...
    def searchContents(self,text):  
        ...
        if directory1!=None:     # contents exist, set the media
            print(directory1[0])
               # check which second it appaer
            storePath=os.getcwd()
            storePath+="/pic2"
            totalSec=final.final(directory1[0],storePath)
            print(totalSec)
            for i in range(1,totalSec+1):
                thelabel=detectv2.detect(i)
                for item in thelabel:
                    if item==text:
                        print(i)
                        self.timeRemider(text,i)
                        break
                else:
                    continue
                break
        ...
        elif directory2!=None:    #contents exist, set the media
            print(directory2[0])
               # check which second it appaer
            storePath=os.getcwd()
            storePath+="/pic2"
            totalSec=final.final(directory2[0],storePath)
            print(totalSec)
            for i in range(1,totalSec+1):
                thelabel=detectv2.detect(i)
                for item in thelabel:
    ...
    def timeRemider(self,obj,time):
        msg = QMessageBox()
        msg.setWindowTitle("Warning")
        msg.setText("Warning")
        msg.setIcon(QMessageBox.Warning)
        msg.setStandardButtons(QMessageBox.Cancel)
        msg.setInformativeText("the "+str(obj)+" appeared from the "+str(time-1)+"second!")
